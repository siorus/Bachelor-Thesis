...
def installed_dist():
  """
  Returns used package manager of distribution
  """
...
def examined_log_file():
  ...
  if installed_dist() == "deb":
    ...
    --------------------------------------
    GUIDE
    This variable holds path to log file 
    when Debian based distro is used
    --------------------------------------
    """
    ...
    GUIDE
    This variable holds path to log file 
    when RHEL based distro is used
    --------------------------------------
    """
    log_file = "/var/log/error_log"
  else:
...
def examine_new_logs(log_file,line_to_start):
  ...
  ----------
  GUIDE
  Strings in this list contain regexes, which will match errors in log file.
  -----------
  """ 
  examing_regex = ["^.*Regex to found.*$","^.*Regex to found2.*$"]  #searching strings
  fp = open(log_file,"r")
  new_lines = 0
  ...
  for line in fp.readlines()[line_to_start:]:
    ...
    for regex in examing_regex:
      try:
        ...
        continue  #current line do not contain searching pattern 
  store_num_of_lines(line_to_start+new_lines)  #save next search line position
  return err_line
  
def main():
  version = 1.0
  parser = argparse.ArgumentParser(prog="rsyslog_example_script.py", description="Looks for errors in logs",formatter_class=argparse.ArgumentDefaultsHelpFormatter)
  ...
  parser.add_argument("-f","--file",help="specifies file which will be examined",action="store",type=str)
