...
def print_output(modified_adapters,importance,script):
  ...
  for key,value in modified_adapters.items():
    if modified_adapters[key][0] == "":  #no previous record of this adapter, it is new
      ret_str = ret_str + "NEW ADAPTER ADDED: " + key + " IPv6 ADDRESS: " + modified_adapters[key][1] + "\n"
    elif modified_adapters[key][1] == "":  #adapter from conf file not in current scan, it was deleted
      ret_str = ret_str + "ADAPTER: " + key + " WAS DELETED IPv6 ADDRESS: " + modified_adapters[key][0] + "\n"
    else:  #adapter ipv6 address was modified
      ret_str = ret_str + "ADAPTER MODIFIED: " + key + " OLD IPv6 ADDRESS: " + modified_adapters[key][0] + " NEW IPv6 ADDRESS: " + modified_adapters[key][1] + "\n"
...
def main():
  version = 1.0
  parser = argparse.ArgumentParser(prog="netcard_ip6_change.py", description="Monitoring IPv6 address on interfaces, address change, new adapters, deleted adpaters",formatter_class=argparse.ArgumentDefaultsHelpFormatter)
  ...
  parser.add_argument("-c","--config",help="config path has to be specified, when linmon monitoring tool is not installed, do not copy linmon_builtin module, standalone only use",type=str)
  ...
  for adapter in adapters:
    try:
      adapter_ip6[adapter] = re.search(".* "+adapter+": (?:.*\n){4} *inet6 *(.*?) .*",ip_out).group(1)  #filter ipv6 address
    except AttributeError:
      adapter_ip6[adapter] = "N/A"  #no address specified
  if stand_alone == True:  #standalone script, linmon not installed
    if not args.config:
      ...
      config_file = args.config + "/netcard_ip6_change.conf"  #config file exist
    else:
    ...
    try:
      config_file = linmon_builtin.read_configfile_fn("config")+"netcard_ip6_change.conf"  #file, where adapter ipv6 addresses are stored
  ...
  if os.path.isfile(config_file):  #config file exist, there is something to compare
    modified_adapters = compare_actual_adapters(adapter_ip6,config_file)  #compare actual ipv6 addresses with last found ipv6
    if modified_adapters == {}:  #nothing has changed from previous scan
      ...
      store_adapters(adapter_ip6,config_file)  #store actual new ipv6 addresses to comparing file
  else:  #first run of script, only create config, no comparison
    store_adapters(adapter_ip6,config_file)
    ret_str = ""
  print(ret_str)
