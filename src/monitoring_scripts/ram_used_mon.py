...
def print_output(actual_value,threshold,free_out,importance,type,script):
  ...
  Keyword arguments:
  actual_value -- current used ram
  threshold -- defined used ram threshold
  ...
  if actual_value >= threshold:
    ret_str = importance + "\n"
    ret_str = ret_str + "THRESHOLD USED RAM: " + str(threshold) + type +"\n"
    ret_str = ret_str + "CURRENT USED RAM: " + str(actual_value) + type +"\n"
...
def main():
  version = 1.0
  parser = argparse.ArgumentParser(prog="ram_used_mon.py", description="Monitoring used ram utilization on machine",formatter_class=argparse.ArgumentDefaultsHelpFormatter)
  ...
  unit_type.add_argument("-p","--percentage",help="threshold used ram in percentage",action="store",type=int)
  unit_type.add_argument("-m","--megabytes",help="threshold used ram in megabytes",action="store",type=int)
  ...
  used = re.search("(.*?) +(\d+) +(\d+) +(\d+) +(\d+) +(\d+)",free_out.split("\n",3)[1]).group(3)
  percentage = int(used)/int(total)*100
  mibibytes = int(used)
