...
def print_output(err_lines,log_file,script,importance):
  ...
  if err_lines != "":
    ret_str = importance + "\n"
    ret_str = ret_str + "Found Apache error(s) in " + log_file + ":\n"
...
def examine_new_logs(log_file,line_to_start):
  ...
  """
  err_line = ""
  examing_regex = ["^.*Resource temporarily unavailable\:.*$","^.*Client sent malformed Host header.*$","^.*/etc/passwd.*$"]  #searching strings
  fp = open(log_file,"r")
  new_lines = 0
...
def main():
  version = 1.0
  parser = argparse.ArgumentParser(prog="rsyslog_apache.py", description="Looks for errors in Apache logs",formatter_class=argparse.ArgumentDefaultsHelpFormatter)
