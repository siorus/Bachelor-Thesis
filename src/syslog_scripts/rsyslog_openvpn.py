...
def print_output(err_lines,log_file,script,importance):
  ...
  if err_lines != "":
    ret_str = importance + "\n"
    ret_str = ret_str + "Found OpenVPN error(s) in " + log_file + ":\n"
...
def examine_new_logs(log_file,line_to_start):
  ...
  """
  err_line = ""
  examing_regex = ["^.*Auth Username/Password verification failed for peer.*$"]
  fp = open(log_file,"r")
  new_lines = 0
...
def main():
  version = 1.0
  parser = argparse.ArgumentParser(prog="rsyslog_openvpn.py", description="Looks for OpenVPN errors in syslog",formatter_class=argparse.ArgumentDefaultsHelpFormatter)
