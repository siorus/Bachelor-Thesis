...
def examine_new_logs(log_file,line_to_start):
  ...
  """
  err_line = ""
  examing_regex = ["^.*Sender address rejected: Improper use of SMTP command pipelining.*$","^.*Insufficient system storage.*$"]  #searching pattern
  fp = open(log_file,"r")
  new_lines = 0
...
def main():
  version = 1.0
  parser = argparse.ArgumentParser(prog="rsyslog_postfix.py", description="Looks for Postfix errors in syslog",formatter_class=argparse.ArgumentDefaultsHelpFormatter)
