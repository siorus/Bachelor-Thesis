...
def print_output(err_lines,log_file,script,importance):
  ...
  if err_lines != "":
    ret_str = importance + "\n"
    ret_str = ret_str + "Found Sendmail error(s) in " + log_file + ":\n"
...
def examine_new_logs(log_file,line_to_start):
  ...
  """
  err_line = ""
  examing_regex = ["^.*due to pre-greeting traffic.*$","^.*Connection rate limit exceeded.*$",\
                  "^.*rejecting connections on daemon MTA: load average.*$","^.*runqueue: Aborting queue run: load average too high.*$"]  #searching pattern
  fp = open(log_file,"r")
  new_lines = 0
...
def main():
  version = 1.0
  parser = argparse.ArgumentParser(prog="rsyslog_sendmail.py", description="Looks for Sendmail errors in syslog",formatter_class=argparse.ArgumentDefaultsHelpFormatter)
  ...
  parser.add_argument("-f","--file",help="specifies file which will be examined, default is /var/log/mail.log or /var/log/maillog depending on distribution",action="store",type=str)
