...
def print_output(err_lines,log_file,script,importance):
  ...
  if err_lines != "":
    ret_str = importance + "\n"
    ret_str = ret_str + "Found Postfix error(s) in " + log_file + ":\n"
...
def examine_new_logs(log_file,line_to_start):
  ...
  """
  err_line = ""
  examing_regex = ["^.*imap-login: Aborted login.*$","^.*pop3-login: Aborted login.*$",\
                  "^.*LDAP: ldap_result() failed: Can't contact LDAP server.*$","^.*LDAP: Can't connect to server.*$"]  #serching patterns
  fp = open(log_file,"r")
  new_lines = 0
...
def main():
  version = 1.0
  parser = argparse.ArgumentParser(prog="rsyslog_dovecot.py", description="Looks for Dovecot errors in syslog",formatter_class=argparse.ArgumentDefaultsHelpFormatter)
