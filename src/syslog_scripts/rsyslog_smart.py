...
def print_output(err_lines,log_file,script,importance):
  ...
  if err_lines != "":
    ret_str = importance + "\n"
    ret_str = ret_str + "Found S.M.A.R.T. error(s) in " + log_file + ":\n"
...
def examine_new_logs(log_file,line_to_start):
  ...
  """
  err_line = ""
  examing_regex = ["^.*SMART Prefailure Attribute.*$","^.*Currently unreadable \(pending\) sectors.*$",\
                   "^.*Offline uncorrectable sectors.*$"]  #searching pattern
  fp = open(log_file,"r")
  new_lines = 0
...
def main():
  version = 1.0
  parser = argparse.ArgumentParser(prog="rsyslog_smart.py", description="Looks for S.M.A.R.T. errors in syslog",formatter_class=argparse.ArgumentDefaultsHelpFormatter)
